...
class Prepare(object):
    ...
    def build_thread_centric_masks(self, replies, speakers, sentence_lengths):
        """
        构建线程中心图结构所需的掩码
        """
        n = len(replies)
        thread_mask = np.zeros((n, n), dtype=np.float32)
        speaker_mask = np.zeros((n, n), dtype=np.float32)

        # 1. 构建线程内连接
        current_thread = []
        for i, reply_to in enumerate(replies):
            if reply_to == -1 or i == 0:  # 新线程开始
                if current_thread:
                    # 线程内全连接
                    for idx1 in current_thread:
                        for idx2 in current_thread:
                            thread_mask[idx1, idx2] = 1.0
                current_thread = [i]
            else:
                current_thread.append(i)
        ...
        for i, speaker in enumerate(speakers):
            speaker_dict[speaker].append(i)

        for speaker, indices in speaker_dict.items():
            for idx1 in indices:
                for idx2 in indices:
                    speaker_mask[idx1, idx2] = 1.0

        # 3. 扩展到token级别
        total_tokens = sum(sentence_lengths)
        token_thread_mask = np.zeros((total_tokens, total_tokens), dtype=np.float32)
        token_speaker_mask = np.zeros_like(token_thread_mask)

        token_offset = 0
        for sent_idx, length in enumerate(sentence_lengths):
            sent_range = slice(token_offset, token_offset + length)
            for sent_idx2, length2 in enumerate(sentence_lengths):
                sent_range2 = slice(token_offset, token_offset + length2)
                if thread_mask[sent_idx, sent_idx2] > 0:
                    token_thread_mask[sent_range, sent_range2] = 1.0
                if speaker_mask[sent_idx, sent_idx2] > 0:
                    token_speaker_mask[sent_range, sent_range2] = 1.0
            token_offset += length

        return token_thread_mask, token_speaker_mask
    ...
    def forward(self):
        ...
        for i, w in enumerate(modes.split()):
            ...
            for line in tqdm(data):
                ...
                sentence_lengths = [len(sent) for sent in dialog['sentences']]

                thread_mask, speaker_mask = self.build_thread_centric_masks(
                    replies, speakers, sentence_lengths
                )

                # 添加新图结构到对话数据
                dialog['thread_mask'] = thread_mask.tolist()
                dialog['speaker_mask'] = speaker_mask.tolist()
